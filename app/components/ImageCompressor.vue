  ...
  <div class="min-h-screen bg-background">
    <header class="border-b border-border">
      <div class="mx-auto flex h-20 max-w-[2000px] items-center justify-between px-6 lg:px-10">
        <h1 class="text-5xl font-black tracking-tighter text-foreground sm:text-6xl lg:text-7xl">Squish</h1>
    ...
    <main class="mx-auto max-w-[2000px] px-6 py-12 lg:px-10 lg:py-16">
      <div class="mb-16 max-w-xl">
        <p class="text-xl leading-relaxed text-muted-foreground">Drop your images below to compress them instantly. Drag the slider on the preview to compare original and compressed versions side by side.</p>
      </div>
      <div class="grid gap-10 xl:grid-cols-[1fr_360px]">
        <div class="space-y-10">
          ...
          >
            <input type="file" accept="image/png,image/jpeg,image/jpg" multiple @change="handleFileSelect" class="absolute inset-0 cursor-pointer opacity-0" aria-label="Select images to compress" />
            <div class="flex flex-col items-center gap-4 px-8 text-center">
              <UIcon name="i-lucide-image" :class="['h-8 w-8 transition-colors', isDragging ? 'text-primary' : 'text-muted-foreground']" />
              <div>
                <p :class="['text-lg font-medium transition-colors', isDragging ? 'text-primary' : 'text-foreground']">{{ isDragging ? 'Release to squish' : 'Drop images here' }}</p>
                <p class="mt-1 text-sm text-muted-foreground">or click to browse — PNG and JPG supported</p>
              </div>
            </div>
          </div>
          <div v-if="selectedImage && selectedImage.compressedUrl" class="space-y-4">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                <h2 class="text-lg font-semibold text-foreground">Preview Comparison</h2>
                <span class="text-sm text-muted-foreground">Drag slider to compare</span>
              </div>
              <button @click="downloadImage(selectedImage)" class="text-sm text-primary transition-colors hover:text-primary/80">Download this image</button>
            </div>
            <ComparisonSlider :key="`${selectedImage.id}-${selectedImage.compressedUrl}`" :original-url="selectedImage.previewUrl" :compressed-url="selectedImage.compressedUrl" :original-size="selectedImage.originalSize" :compressed-size="selectedImage.compressedSize" />
          </div>
          <div v-if="images.length > 0" class="space-y-4">
            <div class="flex items-center justify-between">
              <h2 class="text-lg font-semibold text-foreground">{{ images.length }} {{ images.length === 1 ? 'image' : 'images' }}</h2>
              <button @click="clearAll" class="text-sm text-muted-foreground transition-colors hover:text-foreground">Clear all</button>
            </div>
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
              <div v-for="img in images" :key="img.id" :class="['group relative cursor-pointer overflow-hidden rounded border-2 transition-all', selectedImageId === img.id ? 'border-primary' : 'border-transparent hover:border-muted-foreground']" @click="selectedImageId = img.id" role="button" tabindex="0">
                <div class="aspect-square bg-muted">
                  <img :src="img.previewUrl" :alt="img.originalFile.name" class="h-full w-full object-cover" />
                </div>
                <div v-if="getSavings(img.originalSize, img.compressedSize) > 0" class="absolute top-2 left-2 rounded bg-primary px-1.5 py-0.5 text-xs font-bold text-primary-foreground">-{{ getSavings(img.originalSize, img.compressedSize) }}%</div>
                <div class="absolute inset-0 flex items-center justify-center gap-2 bg-background/80 opacity-0 transition-opacity group-hover:opacity-100">
                  <button @click.stop="downloadImage(img)" class="rounded-full bg-primary p-2 text-primary-foreground transition-colors hover:bg-primary/90" aria-label="Download"><UIcon name="i-lucide-download" class="h-5 w-5" /></button>
                  <button @click.stop="removeImage(img.id)" class="rounded-full bg-destructive p-2 text-destructive-foreground transition-colors hover:bg-destructive/90" aria-label="Remove"><UIcon name="i-lucide-x" class="h-5 w-5" /></button>
                </div>
                <div class="absolute inset-x-0 bottom-0 bg-linear-to-t from-background/90 to-transparent p-2 pt-6">
                  <p class="truncate text-xs text-foreground">{{ formatSize(img.compressedSize) }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-8">
          <div class="space-y-6 rounded border border-border bg-card p-6">
            <div class="flex items-baseline justify-between">
              <h3 class="text-sm font-medium uppercase tracking-widest text-muted-foreground">Quality</h3>
              <span class="text-4xl font-black tabular-nums text-primary">{{ quality }}%</span>
            </div>
            <USlider v-model="quality" :min="10" :max="100" :step="5" color="primary" size="lg" class="w-full" />
            <div class="flex justify-between text-xs text-muted-foreground">
              <span>Smaller file</span>
              <span>Better quality</span>
            </div>
            <QualityIndicator :quality="quality" :savings="totalSavings" />
          </div>
          <div v-if="images.length > 0" class="space-y-6 rounded border border-border bg-card p-6">
            <h3 class="text-sm font-medium uppercase tracking-widest text-muted-foreground">Summary</h3>
            <div class="space-y-4">
          ...
          <div v-if="images.length > 1" class="space-y-4">
            <button @click="downloadAll" class="flex w-full items-center justify-center gap-2 rounded bg-primary px-4 py-3 font-medium text-primary-foreground transition-colors hover:bg-primary/90"><UIcon name="i-lucide-download" class="h-5 w-5" />Download all</button>
          </div>
          <div v-if="isCompressing" class="flex items-center justify-center gap-3 rounded border border-border bg-card p-4">
            <div class="h-5 w-5 animate-spin rounded-full border-2 border-muted-foreground/30 border-t-primary"></div>
    ...
    <footer class="mt-16 border-t border-border">
        ...
        <p class="text-sm text-muted-foreground">All processing happens locally in your browser. Your images never leave your device.</p>
      </div>
    </footer>
  </div>
</template>
